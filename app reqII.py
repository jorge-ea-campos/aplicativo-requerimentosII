...
def display_student_details(df_requerimentos, df_merged, uploaded_hes):
    ...
    for _, aluno in alunos_unicos.iterrows():
        ...
        with st.expander(f"👤 {aluno[COL_NOME]} (NUSP: {nusp_aluno})"):
            ...
            # --- Exibição do Histórico Escolar em PDF ---
            ...
            if current_requests.empty:
                ...
                for index, request in current_requests.iterrows():
                    ...
                    if status != 'Pendente':
                        ...
                        # Salva a justificativa automaticamente ao ser alterada
                        st.session_state.decisions[decision_key]['justificativa'] = justificativa_input
                    else:
                        # Limpa a justificativa se o status voltar para pendente
...
def run_app():
    ...
    st.markdown("""
        <div class="header-container">
            <div class="logo-and-title">
                <img src="https://upload.wikimedia.org/wikipedia/commons/thumb/c/c3/Usp-brasao.svg/1200px-Usp-brasao.svg.png" class="header-logo">
    ...
    with st.sidebar:
        ...
        st.info("💡 Os arquivos 1 e 2 devem ter uma coluna com o número USP.")
    ...
    try:
        with st.spinner("Processando arquivos..."):
            ...
            # --- Processamento dos PDFs de Histórico Escolar ---
            uploaded_hes = {}
            if files_he:
